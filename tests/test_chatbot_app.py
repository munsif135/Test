...
def test_sub_endpoint():
    ...
    response = client.post("/sub", json={"a": 5, "b": 3})
    assert response.status_code == 200
    assert response.json() == {"result": 2}
    
    response = client.post("/sub", json={"a": 3, "b": 3})
    assert response.status_code == 200
    assert response.json() == {"result": 0}
    
    response = client.post("/sub", json={"a": 2, "b": 3})
    assert response.status_code == 200
    assert response.json() == {"result": -1}

def test_mul_endpoint():
    """Test the /mul endpoint with various inputs"""
    response = client.post("/mul", json={"a": 2, "b": 3})
    assert response.status_code == 200
    assert response.json() == {"result": 6}
    
    response = client.post("/mul", json={"a": -1, "b": 1})
    assert response.status_code == 200
    assert response.json() == {"result": -1}
